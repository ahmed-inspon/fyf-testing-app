    ...
    if(!window.fyf_app_extension){
        ...
        const base_url = "https://7e47-182-189-249-148.eu.ngrok.io/";
        let store_name = null;
        let product_id = null;
        let sizes_order = [
            "3XL","2XL","XL","L","M","S","XS"
        ...
        if(document.querySelector(".fyf-extension")){
            store_name = document.querySelector(".fyf-extension").getAttribute("data-store-name");
            product_id = document.querySelector(".fyf-extension").getAttribute("data-product-id");
    ...
    let sleeves_input = document.querySelector(".fyf-extension input[name='sleeves']");
    let shirt_length_input = document.querySelector(".fyf-extension input[name='shirt_length']");
    ...
    const init = async ()=>{
        ...
        if(resp.ok){
            ...
            if(resp.success){
                ...
                if(measurements){
                    let best_fit = localStorage.getItem("fyf-best-fit-"+measurements.id);
                    if(best_fit){
                    ...
                    document.querySelectorAll(".fyf-modal-snippet .chart-head").forEach((el)=>{
                        el.innerHTML = table_head;
                    })
                    document.querySelectorAll(".fyf-modal-snippet .chart-body").forEach((el)=>{
        ...
        // Get the modal
        var modal = document.getElementById('fyf-modal');
    ...
    document.querySelector('.fyf-extension #step-2')?.addEventListener('click',async ()=>{
        let unit = document.querySelector(".fyf-extension input[type='radio'][name='unit']:checked").value;
        ...
        let sleeves = unit_conversion(sleeves_input.value,unit,measurement_unit);
        let shirt_length = unit_conversion(shirt_length_input.value,unit,measurement_unit);
    ...
    if(document.querySelector(".fyf-app-embed")){
        // alert("added");
        let variants = document.querySelector("variant-radios");
        let embed_block = document.querySelector(".fyf-app-embed");
        variants.parentNode.insertBefore(embed_block, variants.nextSibling)

        console.log("variants",variants,embed_block);
    }
    const mode = (arr) => {
        let sizes_count = {};
        let max_count =  0;
        ...
        console.log("arr",arr);
        if(arr.length == 3){ // three graph measurement
        ...
        if(arr.length == 4){
            ...
            if(max_count >= 3){ // two suggested sizes with 1 mode [S,M,M,M] || [M,M,M,M]
                return sortable_size[0][0];
            }
            else if(max_count == 2){ // [S,M,L,L] || [S,S,M,M]
                if(sortable_size.length == 2) // two suggested sizes with 2 modes [S,S,M,M]
                {
                    for(let i = 0;i < sizes_order.length ; i++){ // selecting high size among 2 modes
                        if(arr.includes(sizes_order[i])){
                            return sizes_order[i];
                        }
                    }
    ...
    const unit_conversion = (num,from,to) =>{
        ...
        {   
            if(to == "inches" || to == "inch" || to == "in"){
                converted = Math.ceil(num * Cmfactor);
            }
        }
        return converted;
    }
